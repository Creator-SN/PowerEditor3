<template>
	...
	>
		...
		<span
			...
			v-html="
				thisValue === '' ? node.attrs.emptyPlaceholder : equationString
			"
			ref="target"
			:style="{
				color: `${lock ? 'red' : ''}`,
	...
	methods: {
		...
		render() {
			try {
				...
				this.equationString = katex.renderToString(
					this.thisValue.replace(/\\tag\{.*?\}/g, ""),
					{
						throwOnError: true,
		...
		close(confirm = true) {
			...
			let selection = null;
			if (this.$refs.input.selectionEnd === 0)
...
.power-editor-equation-container {
	...
	.power-editor-equation-popper-fade-enter-from {
		transform: scale(0);
		opacity: 0;
	}
	...
	.power-editor-equation-popper-fade-leave-from {
		transform: scale(1);
	}
